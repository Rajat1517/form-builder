...
function Lifter({ builders, handleLogin, handleSignOut, isAuthenticating }: { builders: BiulderState[]; handleLogin: MouseEventHandler<HTMLButtonElement>; handleSignOut: MouseEventHandler<HTMLButtonElement>; isAuthenticating: boolean; }) {
  ...
  return (
    <div className={`${styles.container}`}>
      ...
      <div>
        {!user && <button className={styles.googleLogo} disabled={isAuthenticating} title="Sign In with Google" onClick={handleLogin}><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 48 48">
        ...
        {user && <Profile user={user} handleSignOut={handleSignOut} />}
      </div>
    </div>
  )
}
